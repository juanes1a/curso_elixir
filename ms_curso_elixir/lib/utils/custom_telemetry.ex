defmodule MsCursoElixir.Utils.CustomTelemetry do
  alias MsCursoElixir.Utils.DataTypeUtils
  alias MsCursoElixir.Utils.CustomTelemetry
  import Telemetry.Metrics

  @service_name Application.compile_env!(:ms_curso_elixir, :custom_metrics_prefix_name)
  ...
  def custom_telemetry_events() do
    :telemetry.attach("ms_curso_elixir-plug-stop", [:ms_curso_elixir, :plug, :stop], &CustomTelemetry.handle_custom_event/4, nil)
    :telemetry.attach("ms_curso_elixir-vm-memory", [:vm, :memory], &CustomTelemetry.handle_custom_event/4, nil)
  ...
  def handle_custom_event([:ms_curso_elixir, :plug, :stop], measures, metadata, _) do
    :telemetry.execute(
